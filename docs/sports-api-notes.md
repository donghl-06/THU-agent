...
## 排障史（2026-09-02，第四次：错别字场景名 + uuid 数据重排）
...
**发现 2：8-31 晚服务端数据重排，ROOM 节点 uuid 全部变化**
（气膜馆 0fdca206... → 1865781826241564673）。硬编码旧 uuid 的探针/current/page
全部返回空壳；实时级联的查询不受影响。用旧 uuid 做诊断会得出"接口失效"的假结论。
教训：**探针不要硬编码 uuid**（本次 scripts/debug-*.ts 系列已全部改为实时级联）。
