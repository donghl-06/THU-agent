...
- 默认暴露 11 个校园只读工具：课表、校园卡、教室、图书馆座位/研讨间、体育场馆、成绩单、宿舍电费、宿舍卫生、校园网状态和我的图书馆预约。宿舍卫生返回公示图，校园网查询另需 `CJY_*` 验证码配置。
...
- MCP 不装配提醒、定时抢场和任务管理工具。需要全部 22 个工具时，可使用[项目级 Agent Skill / JSON CLI](../README.md#供任意-ai-agent-调用)；任务执行仍依赖常驻 Web 服务，详见[项目架构](architecture.md)。
