...
- 默认暴露 25 个校园只读工具：课表、校园动态/资讯、校园卡、教室、图书馆、体育场馆、成绩单、宿舍电费、宿舍卫生、校园网状态、网络学堂、清华邮箱与清华云盘。宿舍卫生返回公示图，校园网查询另需 `CJY_*` 验证码配置；邮箱另需 `THU_EMAIL_*` 配置。
- 云盘只读包含列资料库、浏览目录、搜索文件，以及用 `show_cloud_file` 获取图片、视频、音频、普通文件的临时访问链接；MCP 环境没有 Web 受控预览通道，文件由调用方或用户自行按需加载。
- 云盘上传、新建、重命名、移动、复制、删除、生成分享链接属于写操作，默认不暴露给 MCP；请使用 Web/EXE 的确认界面执行。
- 邮件和学堂图片工具需要 Web 临时图片通道；MCP 中会返回 `NOT_SUPPORTED`。
...
- MCP 不装配提醒、定时抢场和任务管理工具。需要全部 45 个工具时，可使用[项目级 Agent Skill / JSON CLI](../README.md#供任意-ai-agent-调用)；任务执行仍依赖常驻 Web 服务，详见[项目架构](architecture.md)。
...
将路径替换为用户实际的 MCP 包目录。配置修改后重启 Agent。在新会话中可以先询问“调用 `get_user_info` 检查我的清华登录状态”，再尝试“查询我今天的课表”或“最近有什么校园资讯？”。
