...
For candidate or ambiguity errors, present the returned choices and ask the user to select one. For payment results that contain a payment link or QR code, explain that completing the returned payment flow is still the user's action; do not claim payment succeeded unless the capability explicitly reports that outcome.

Campus-card recharge also supports a bank channel that directly requests a debit from the campus-card system's linked bank account, with no QR code. Use it only when the user explicitly selects bank payment and approves the exact amount. Do not silently replace a QR payment with bank payment. A bank result with `paymentStatus: "submitted"` confirms submission only, not a successful debit or credited balance. Check the campus-card balance and ask the user to reconcile recharge/bank records when needed. For `PAYMENT_STATUS_UNKNOWN`, do not automatically retry or switch payment channels; reconcile the earlier payment first. This single recharge capability does not register a balance monitor or authorize recurring payments.
