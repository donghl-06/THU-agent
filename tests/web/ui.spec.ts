...
test("云盘视频和文件渲染为可清理的本地预览卡片", async ({page}) => {
    ...
    await expect(fileCard.getByRole("button", {name: /测试讲义\.pdf/})).toHaveAttribute("title", "点击在新窗口预览");
    await expect(fileCard.getByRole("link", {name: "新窗口预览"})).toHaveCount(0);
    ...
    expect(previewPage.url()).toContain(fileUrl);
