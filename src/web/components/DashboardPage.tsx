...
import {ArrowUpRight, Bell, BookOpen, CalendarDays, Check, ChevronDown, CreditCard, ExternalLink, GraduationCap, LayoutDashboard, Library, LoaderCircle, LockKeyhole, Mail, Megaphone, Newspaper, RefreshCw, Search, Sparkles, Wifi, Zap, type LucideIcon} from "lucide-react";
...
const icons: Record<string, LucideIcon> = {card: CreditCard, electricity: Zap, network: Wifi, homework: BookOpen, notices: Megaphone, schedule: CalendarDays, news: Newspaper, bookings: CalendarDays, seats: Library, courses: GraduationCap, files: BookOpen, calendar: CalendarDays, emails: Mail, report: GraduationCap, rooms: Library, dorm: Sparkles};
...
const panelOrder = ["homework", "notices", "schedule", "news", "card", "electricity", "network", "bookings", "seats", "courses", "files", "calendar", "emails", "report", "rooms", "dorm"];
...
function DashboardWorkspace({sidebarToggle}: {sidebarToggle?: ReactNode}) {
    ...
    return <motion.div initial="hidden" animate="visible" variants={staggeredReveal}>
        ...
        <motion.div className="dashboard-overview" variants={contentReveal}>
            {[{id: "card", label: "校园卡余额", icon: CreditCard}, {id: "electricity", label: "电费余额", icon: Zap}, {id: "homework", label: "待交作业", icon: BookOpen}, {id: "schedule", label: "今日课程", icon: CalendarDays}].map(({id, label, icon: Icon}) => {
...
function StatusPanel({panel, refresh, busy, open}: {panel: DashboardPanel; refresh: () => void; busy: boolean; open: (item: DashboardItem) => void}) {
    ...
    return <article className="dashboard-panel" id={`dashboard-${panel.id}`} aria-labelledby={`dashboard-title-${panel.id}`}>
        ...
        {!panel.data && !error ? <div className="dashboard-pending"><LoaderCircle size={16} className="dashboard-spin"/>{panel.refreshing ? "正在查询…" : "等待查询"}</div> : <>
            {items.length > 0 ? <ul className="dashboard-list">{items.slice(0, expanded ? items.length : 5).map((item, index) => <li key={`${item.title}-${index}`}>
        ...
        <footer><span>{panel.updatedAt ? `${formatTime(panel.updatedAt)} 更新` : "尚无成功记录"} · 每 {panel.intervalMs / 60_000} 分钟</span>{items.length > 5 && <button aria-expanded={expanded} onClick={() => setExpanded(!expanded)}>{expanded ? "收起" : `查看全部 ${items.length} 条`}<ChevronDown size={13} className={expanded ? "is-expanded" : ""}/></button>}</footer>
    </article>;
}
